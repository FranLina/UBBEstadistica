...
class PlantillaVisitanteFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val equipoVisitante = prefs.getString("EquipoVisitante", "").toString()
        val idPartido = prefs.getString("idPartido", "").toString()

        listView = binding.LVPlantillaVisitante

        db.collection("Equipos").document(equipoVisitante).get().addOnSuccessListener {
            binding.txtPlantillaEquipoV.text = it.get("Nombre").toString().toUpperCase(Locale.ROOT)
            ...
            recuperaJugadores(root, equipoVisitante, it.get("Jugadores") as HashMap<String, String>)
        }
